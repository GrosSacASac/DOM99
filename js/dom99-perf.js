...
// soft arrays with length 1 are the values
// soft arrays with length 0 or more can become normal Arrays
const forEachSoft = function (func, ArrayOrAtom) {
    if (!Array.isArray(ArrayOrAtom)) {
        func(ArrayOrAtom);
    } else {
        ArrayOrAtom.forEach(func);
    }
};

const pushOrCreateSoft = function (object, key, valueToPush) {
// replaces pushOrCreateArrayAt
        if (!hasOwnProperty.call(object, key)) {
            // eventually the if is always true
            object[key] = valueToPush;
        } else if (hasOwnProperty.call(object, key)) {
            object[key] = [object[key], valueToPush];
        } else /*if (Array.isArray(object[key]) && object[key].length >= 2) */ {
...
const d = (function () {
    ...
    const notifyVariableSubscribers = function (subscribers, value) {
        ...
        forEachSoft(function (variableSubscriber) {
    ...
    const notifyListSubscribers = function (subscribers, startPath, data) {
        forEachSoft(function (listContainer) {
    ...
    const applylist = function (element, attributeValue) {
        ...
        pushOrCreateSoft(listSubscribers, path, element);
    ...
    const applyVariable = function (element, variableName) {
        ...
        const path = contextFromArrayWith(pathIn, variableName);
        pushOrCreateSoft(variableSubscribers, path, element);
